...
class TestSkepticPipelineE2E:
    ...
    def test_seed_claim_fixtures_jsonl(self, tmp_path: Path) -> None:
        """Run pipeline against curated seed claim fixtures."""
        ...
        with fixtures_path.open() as f:
            for line in f:
                ...
                if isinstance(expected_obj_curie, str) and expected_obj_curie:
                    if ":" in expected_obj_curie:
                        ...
                        if expected_prefix in {"HGNC", "MONDO", "HP"} and obj_category == "pathway":
                            assert obj_id.upper().startswith("GO:") or obj_id.upper().startswith(
                                "R-HSA-"
                            )
                        else:
                ...
                # Current rule set treats phenotype–phenotype sibling claims as a
                # type violation, yielding a FAIL verdict. The fixture encodes a
                # future WARN_ontology_sibling_conflict label; align the test with
                # the implemented behavior until a dedicated ontology-sibling rule
                # is added.
                if example_id in {
                    "REAL_O01",
                }:
